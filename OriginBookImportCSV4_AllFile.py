...
def main():
    ...
    try:
        ...
        csv_lists = processor.get_csv_files(config['data_file'])
        print(csv_lists)
        for data_item in csv_lists:

            df, columns, success = processor.read_data_file(data_item, need_columns)
            if not success:
                return False
            ...
            data_basename = os.path.splitext(os.path.basename(data_item))[0]
            output_dir = config['output_dir']
            workbook_file = os.path.join(output_dir, f"{data_basename}.ogw")
            png_output_path = os.path.join(output_dir, f"{data_basename}_plot.png")

            try:
                processor.wb.save(workbook_file)
                print(f"✅ 工作簿已保存: {workbook_file}")
            except Exception as e:
                print(f"⚠️  保存工作簿失败: {e}")
